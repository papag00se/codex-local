...
- [Model Settings](model-settings.md) — Recommended per-model settings across three layers (server launcher / config.toml roles / per-request), precedence, and the per-model reasoning ON/OFF mechanism. The models under test + how to drive them.
- [Local Model Services](local-model-services.md) — The 4 local model systemd services on this rig: launch, switch (one at a time on :18084), boot default, reasoning toggle.
...
- [Heuristic Assists — the catalog](heuristic-assists.md) — Every assist, name + one line each, in five families: **Nudges** (repetition, forced-diagnosis, tunnel-vision, read/search/fetch guards, …), **Massages** (write_file↔shell base64, leaked-call recovery, apply_patch repair, …), **Context Shaping** (window auto-detect, real-token calibration, workspace-manifest pin, …), **Probes** (repo-diagnostic ground-truth completion gate), and **Reasoned Guidance**. The marketable "everything Shephard does".
